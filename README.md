...
int main() {
    ...
    const char * vertexShaderSource = loadShaderSource("shaders/vertex_shader.glsl").c_str();
    const char * fragmentShaderSource = loadShaderSource("shaders/fragment_shader.glsl").c_str();
...
Command Line
```bash
g++ -std=c++11 -static-libgcc -static-libstdc++ -DGLEW_STATIC -o OpenGL_OBJ_Loader src/main.cpp -lglfw3 -lglew32s -lopengl32 -lgdi32 -luser32 -lkernel32 -lshell32
```
or 
```bash
